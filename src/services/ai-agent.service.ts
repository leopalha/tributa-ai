...
export class AIAgentService {
  ...
  private async generateResponse(
    ...
    try {
      ...
      // Usar API real (OpenAI ou similar)
...
- Apresente a solução/ação
- Ofereça alternativas quando possível
- Termine com pergunta para continuidade
...
CONFIANÇA: ${intent.confidence * 100}%
...
Responda como ARIA, oferecendo uma solução prática e executável.
`;
  }
  ...
  private async callServiceMethod(action: AIAction): Promise<any> {
    switch (action.service) {
      ...
      case 'analytics':
        return await this.analyticsService[action.method](action.parameters);

      case 'navigation':
        // Para navegação, retornar URL ou comando
  ...
  private async callAIAPI(systemPrompt: string, userPrompt: string): Promise<string> {
    // Em produção, implementar chamada real para OpenAI/Anthropic
    // Por ora, usar fallback mock
